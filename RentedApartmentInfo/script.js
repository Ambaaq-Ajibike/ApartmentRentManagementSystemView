...
let getButton = document.querySelector(".show-btn");
   ...
   else if(localStorage.getItem("CustomerId") != undefined)
    {
      ...
      getButton.innerHTML = ` 
      <button id="location" style="margin-left: 10pc; background-color:rgb(240, 255, 244)" type="submit" class="btn btn-a"> <a href="/RentedApartments/index.html">Back</a></button>`
      load = document.querySelector("#btn");
    }
 console.log("4")
...
let fetchLandlordInfo = async (landlordId) => {
    let fetchUnApprovedApartmentInfo = await fetch(`https://localhost:7134/api/Customer/GetCustomer/${landlordId}`);
    let jsonUnApprovedApartmentInfo = fetchUnApprovedApartmentInfo.json();
    return jsonUnApprovedApartmentInfo;
}
